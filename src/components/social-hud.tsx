import Image from 'next/image';

export function SocialHud() {
    return (
        <div className="fixed z-99 bottom-0 right-0 pb-8 pr-8">
            <a href="mailto:nicholas.cannon1@gmail.com" className="mt-8 flex justify-center items-center">
                <Image src="/images/socials/email.svg" width={24} height={24} alt="email" />
            </a>
            <a
                href="https://www.linkedin.com/in/niccannon1"
                target="_blank"
                rel="noopener noreferrer"
                className="mt-8 flex justify-center items-center"
            >
                <Image src="/images/socials/linkedin.svg" width={24} height={24} alt="linkedin" />
            </a>
            <a
                href="https://github.com/nicholascannon"
                target="_blank"
                rel="noopener noreferrer"
                className="mt-8 flex justify-center items-center"
            >
                <Image src="/images/socials/github.svg" width={24} height={24} alt="github" />
            </a>
        </div>
    );
}
